> Historical hosted architecture, superseded by the local runtime. See the root README.md and PLAN.md.
...
For a presenter-run demo, a local stdio MCP server (`apps/mcp-server/dist/stdio.js`) calls the Convex dev deployment anonymously, uploads local PDFs/transcripts, and Convex runs the live Grok council. Enable it with `pnpm demo:enable` (sets `DEMO_OPEN_ACCESS=true` and live analysis on the dev deployment), build with `pnpm demo:build`, and disable it after the demo with `pnpm demo:disable`. While enabled, anyone with the deployment URL can use the shared demo workspace and start paid runs. See [docs/DEMO.md](docs/DEMO.md).
